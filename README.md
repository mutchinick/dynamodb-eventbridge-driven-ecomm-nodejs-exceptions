...
```json
"config": {
  "deployment_prefix": "ddbEbEcommNjsExc"
},
```
...
For a `deployment_prefix` of `ddbEbEcommNjsExc` and a `stage` of `dev`, the profile name would be `ddbEbEcommNjsExc-dev`.

_Example `~/.aws/credentials`:_

```ini
[ddbEbEcommNjsExc-dev]
