...
function buildMockListOrderPaymentsApiService_resolves(): IListOrderPaymentsApiService {
  return { listOrderPayments: jest.fn().mockResolvedValue(mockServiceOutput) }
}

function buildMockListOrderPaymentsApiService_throws(error?: unknown): IListOrderPaymentsApiService {
...
describe(`Payments Service ListOrderPaymentsApi ListOrderPaymentsApiController tests`, () => {
  ...
  it(`does not throw if the input APIGatewayProxyEventV2 is valid`, async () => {
    const mockListOrderPaymentsApiService = buildMockListOrderPaymentsApiService_resolves()
  ...
  it(`responds with 500 Internal Server Error if
      ListOrderPaymentsApiService.listOrderPayments throws an Error not accounted for`, async () => {
    const mockError = new Error('mockError')
    const mockListOrderPaymentsApiService = buildMockListOrderPaymentsApiService_throws(mockError)
    const listOrderPaymentsApiController = new ListOrderPaymentsApiController(mockListOrderPaymentsApiService)
    const mockApiEventBody = buildMockApiEventBody()
    const mockApiEvent = buildMockApiEvent(mockApiEventBody)
    const response = await listOrderPaymentsApiController.listOrderPayments(mockApiEvent)
  ...
  it(`responds with 500 Internal Server Error if
      ListOrderPaymentsApiService.listOrderPayments throws an UnrecognizedError`, async () => {
    const unrecognizedError = UnrecognizedError.from()
    const mockListOrderPaymentsApiService = buildMockListOrderPaymentsApiService_throws(unrecognizedError)
    const listOrderPaymentsApiController = new ListOrderPaymentsApiController(mockListOrderPaymentsApiService)
    const mockApiEventBody = buildMockApiEventBody()
    const mockApiEvent = buildMockApiEvent(mockApiEventBody)
    const response = await listOrderPaymentsApiController.listOrderPayments(mockApiEvent)
