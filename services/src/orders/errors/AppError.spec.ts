...
const testCases: ErrorTestCase<AppError>[] = [
  ...
  {
    name: 'StaleOrderStatusTransitionError',
    from: (cause, message) => StaleOrderStatusTransitionError.from(cause, message),
    expectedMessage: 'Stale order status transition error.',
    expectedTransient: false,
    expectedInstance: StaleOrderStatusTransitionError,
  },
  {
