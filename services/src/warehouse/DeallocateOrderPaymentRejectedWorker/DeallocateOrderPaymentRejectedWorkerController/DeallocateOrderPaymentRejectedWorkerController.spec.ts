...
function buildMockDeallocateOrderPaymentRejectedWorkerService_throws(
  error: AppError,
...
describe(`Warehouse Service DeallocateOrderPaymentRejectedWorker DeallocateOrderPaymentRejectedWorkerController tests`, () => {
  ...
  it(`returns an empty SQSBatchResponse.batchItemFailures if the input SQSEvent is undefined`, async () => {
    ...
    const result = await deallocateOrderPaymentRejectedWorkerController.deallocateOrdersStock(mockSqsEvent)
  ...
  it(`returns an empty SQSBatchResponse.batchItemFailures if the DeallocateOrderPaymentRejectedWorkerService 
    ...
    const mockDeallocateOrderPaymentRejectedWorkerService =
      buildMockDeallocateOrderPaymentRejectedWorkerService_throws(nonTransientError)
    const deallocateOrderPaymentRejectedWorkerController = new DeallocateOrderPaymentRejectedWorkerController(
      mockDeallocateOrderPaymentRejectedWorkerService,
    )
  ...
  it(`returns expectedResponse SQSBatchResponse.batchItemFailures if the DeallocateOrderPaymentRejectedWorkerService 
      throws a transient Error (test 1)`, async () => {
    const transientError = InvalidOperationError.from('transient')
    const mockDeallocateOrderPaymentRejectedWorkerService =
      buildMockDeallocateOrderPaymentRejectedWorkerService_throws(transientError)
    const deallocateOrderPaymentRejectedWorkerController = new DeallocateOrderPaymentRejectedWorkerController(
      mockDeallocateOrderPaymentRejectedWorkerService,
    )
    const mockIds = ['AA-THROW', 'BB-THROW', 'CC']
  ...
  it(`returns all SQSBatchResponse.batchItemFailures if the DeallocateOrderPaymentRejectedWorkerService
